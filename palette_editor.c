...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (IsKeyPressed(KEY_C)) { arrfree(palette); Palette_init(&palette, STOCK_COLDHOT1); }
        if (IsKeyPressed(KEY_V)) { arrfree(palette); Palette_init(&palette, STOCK_COLDHOT2); }
        if (IsKeyPressed(KEY_B)) { arrfree(palette); Palette_init(&palette, STOCK_COLDHOT3); }
